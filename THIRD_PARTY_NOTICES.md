...
| --- | --- | --- | --- |
| [FFmpeg](https://ffmpeg.org/) | **Direct container runtime.** Installed from the Debian repository in the pinned Python 3.12 base build. Each image writes its actual version, compiler, configuration flags, and library versions to `/usr/share/doc/nightingale/ffmpeg-build.txt`. The verified development host used `8.0.1`, Apple clang 17, `--enable-version3 --enable-gpl`, x264/x265, libopus, and the other flags printed by `ffmpeg -version`; that host record is not substituted for the container record. | Argument-array subprocess for 16 kHz mono PCM, high-pass/loudness processing, and measurable review signals. | FFmpeg licensing depends on the exact recorded build and enabled libraries. The observed host is GPL-enabled. Release operators must archive the image record, corresponding package/source offer, and notices for the distributed Debian build. |
