...
class EntryVersion(TenantRow, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    entry_id: uuid.UUID = Field(foreign_key="entries.id", index=True)
...
class EntryRelation(TenantRow, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    source_entry_id: uuid.UUID = Field(foreign_key="entries.id", index=True)
    target_entry_id: uuid.UUID = Field(foreign_key="entries.id", index=True)
...
class Comment(TenantRow, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    entry_id: uuid.UUID = Field(foreign_key="entries.id", index=True)
    entry_version_id: uuid.UUID = Field(foreign_key="entry_versions.id", index=True)
    ...
    assigned_membership_id: uuid.UUID | None = Field(
        default=None, foreign_key="clinic_memberships.id"
    )
...
class CommentMention(TenantRow, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    comment_id: uuid.UUID = Field(foreign_key="comments.id", index=True)
...
class CareTask(TenantRow, table=True):
    ...
    assignee_membership_id: uuid.UUID = Field(foreign_key="clinic_memberships.id")
...
class Highlight(TenantRow, table=True):
    ...
    patient_id: uuid.UUID = Field(foreign_key="patients.id", index=True)
    entry_id: uuid.UUID = Field(foreign_key="entries.id", index=True)
    source_entry_version_id: uuid.UUID = Field(
        foreign_key="entry_versions.id", index=True
    )
...
class ProvenancePointer(TenantRow, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    highlight_id: uuid.UUID | None = Field(default=None, foreign_key="highlights.id")
    comment_id: uuid.UUID | None = Field(default=None, foreign_key="comments.id")
    entry_version_id: uuid.UUID = Field(foreign_key="entry_versions.id", index=True)
    start_offset: int
    end_offset: int
...
class ConflictCase(TenantRow, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    patient_id: uuid.UUID = Field(foreign_key="patients.id", index=True)
    left_entry_id: uuid.UUID = Field(foreign_key="entries.id")
    right_entry_id: uuid.UUID = Field(foreign_key="entries.id")
