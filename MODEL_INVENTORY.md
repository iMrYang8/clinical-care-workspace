...
| Voice transcription | Local-first selection | **Default.** | `VOICE_TRANSCRIPTION_PROVIDER=local`; a non-empty, pre-cached `LOCAL_ASR_MODEL_DIR` is required to produce a transcript. | Ordinary audio never leaves the clinic by default. A missing local runtime or weight directory yields an explicit `needs_review` state; no transcript is invented. |
...
| Voice transcript fixture | `trilingual-intrasentential-v1` synthetic provider | Same demo gates as overlap. **Not** default. Flag `VOICE_MULTI_AGENT_PIPELINE` still defaults off. | Same demo gates; exact fixture ID `trilingual-intrasentential-v1`. | Fixed text: clinician EN metformin, patient ZH penicillin denial, family Malay+Hokkien+English `Dia tui penicillin koe-bin masa kecil.` Never ASR. Not on `release-2026-09-02`. |
...
| Voice transcription | faster-whisper `1.2.1`, CTranslate2 `4.8.1`, PyAV `18.1.0` | Default provider choice, optional `local-asr` runtime profile; all three are lock-resolved but not installed in the base image and no weights are bundled. | Build optional group, mount a non-empty pre-cached `LOCAL_ASR_MODEL_DIR`; provider uses CPU/int8 and `local_files_only=True`. | Process timeout/cancellation and adapter contracts are implemented. CTranslate2/PyAV runtime import and a particular weight set remain **not tested**. PyAV wheel FFmpeg composition is release-platform-specific. No diarization is claimed. |
| Speaker/overlap experiment | pyannote.audio 4.0.7 | Experimental optional profile; no model/token bundled. | Accepted model terms, pre-cached `PYANNOTE_MODEL_DIR`, `INSTALL_DIARIZATION=true`, and explicit enablement. | Worker can apply a local pipeline when those gates pass. This repository contains **no measured diarization quality**; overlap remains a review signal, not blind-source separation. |
...
rejected. The committed record was generated on 2026-08-28 from source commit
`0c437fe15f6a68ce511f87a5f07dd46685a813cd` and local release-candidate image
ID `sha256:3370e248f67bb105675eabd2bb8015d53748f4a85618fe32edb064a00311958f`:
FFmpeg `7.1.5-0+deb13u1`, Debian amd64, with `--enable-gpl`; its file SHA-256
is `97a8037eaa36774a22e47f56ed9bdb3a41cb4a5dd4d27e4a52619a1e478e86ca`.
