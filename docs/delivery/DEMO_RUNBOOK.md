# Nightingale delivery and verification runbook
...
## 2. Scenario A — Current priorities to exact source

1. Sign in through the clinical form as **Care staff**. Automated browser tests create this development session through the development authentication endpoint.
2. Open **Alex Tan**.
3. Confirm Current priorities shows concise risk/follow-up cards and the Care timeline shows the
   formal AI Doctor Consult, AI Nurse Consult, and AI Patient Session types.
4. On **AI doctor draft requires clinician review**, select **View source**.
5. Confirm the Source details dialog highlights **clinician review is
   required** in the `ai_doctor_consult_summary` Care timeline entry, rather than
   merely showing that an AI summary exists elsewhere on the page.

Evidence: `frontend/tests/scenarios.spec.ts`, `[Scenario A]` verifies the AI
entry type, resolved immutable version, and highlighted span.

## 3. Scenario B — collaboration, versions, learning, audit

1. As **Care staff**, open Alex's care note.
2. Open **Team discussion** on **Current care review**. Show the seeded
   `@clinician` mention and assignment.
3. Open **Change history** on **Medication reconciliation**. Compare Version 1 and
   Version 2, then select **Restore this version** on Version 1. The older
   version is not deleted; a new current version is created.
4. In Current priorities, show the medication item and its **Clinician accepted** reason.
5. Sign out, sign in as **Clinic admin**, open **Patients** in read-only oversight, and show
   clinic-scoped oversight of Alex's Care timeline, internal comments, versions,
   Current priorities, and provenance. Confirm edit, recording, comment, assignment, and
   highlight decision controls are absent.
6. Return to **Administration**, open the Activity log, and show
   the “Earlier note restored” activity without clinical body text in the audit payload.
...
1. Sign in as **Clinician** and open Alex.
2. Show entries dated **2025-04-15** and **2026-02-06** in one Care timeline.
...
1. Sign in through the separate patient portal and show **My Care · Alex Tan**.
2. The Patient view contains patient-facing Care timeline/Current priorities data only. It does
...
The browser test marks its development-only session as the fixed
`code-switch-overlap-v1` test recording through an intercepted development request. No fixture or provider control is exposed in the product interface. It exercises speaker, timestamp, language,
...
Current priorities performance:
...
The benchmark uniquely resolves **Alex Tan** rather than trusting list
...
## 10. Historical recording workflow

The checked-in recording automation is retained as historical delivery evidence. Its selectors are not part of this productization change; record the final walkthrough manually against the current clinical and patient portals.
...
The walkthrough logs in as Staff, resolves Alex Tan's Current priorities card to the
